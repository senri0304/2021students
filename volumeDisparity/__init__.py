...
# ls
def stereogramize(vol):
    img = Image.new("RGB", (sz, sz), (lb, lb, lb))
    draw = ImageDraw.Draw(img)

    draw.rectangle((int(sz / 2) - int(ll / 2), int(sz / 2) + int(f / 2) * vol,
                    int(sz / 2) + int(ll / 2), int(sz / 2) - int(f / 2) * vol),
                   fill=(0, 0, 0), outline=None)

    fixation(draw)

    basename = os.path.basename(str(vol) + 'ls.png')
