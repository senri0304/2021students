...
def stereogramize(d):
    ...
    basename = os.path.basename('ver' + str(d) + 'r.png')
    img.save(os.path.join(to_dir, basename), quality=100)
    basename = os.path.basename('ver' + str(d) + 'l.png')
    img2.save(os.path.join(to_dir, basename), quality=100)


def stereogramize2(d):
    img = Image.new("RGB", (sz, sz), (lb, lb, lb))
    draw = ImageDraw.Draw(img)
    img2 = Image.new("RGB", (sz, sz), (lb, lb, lb))
    draw2 = ImageDraw.Draw(img2)

    draw.rectangle((int(sz / 2) + int(ll / 2) - d, int(sz / 2) + int(f / 2),
                    int(sz / 2) - int(ll / 2) - d, int(sz / 2) - int(f / 2)),
                   fill=(int(lb*1.5), 0, 0), outline=None)
    draw2.rectangle((int(sz / 2) - int(ll / 2) + d, int(sz / 2) - int(f / 2),
                     int(sz / 2) + int(ll / 2) + d, int(sz / 2) + int(f / 2)),
                    fill=(int(lb*1.5), 0, 0), outline=None)
    ...
    basename = os.path.basename('hor' + str(d) + 'r.png')
    img.save(os.path.join(to_dir, basename), quality=100)
    basename = os.path.basename('hor' + str(d) + 'l.png')
    img2.save(os.path.join(to_dir, basename), quality=100)
