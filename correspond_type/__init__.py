...
# lsh
def line_resize(length):
    img = Image.new("RGB", (sz, sz), (lb, lb, lb))
    draw = ImageDraw.Draw(img)

    draw.rectangle((int(sz / 2) - int(ll / 2)*length, int(sz / 2) + int(f / 2),
                    int(sz / 2) + int(ll / 2)*length, int(sz / 2) - int(f / 2)),
                   fill=(0, 0, 0), outline=None)

    fixation(draw)

    basename = os.path.basename(str(length) + 'ls.png')
    img.save(os.path.join(to_dir, basename), quality=100)
