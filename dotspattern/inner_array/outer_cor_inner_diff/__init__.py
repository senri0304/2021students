...
def draw_dot(d, y_pos, t):
    d.rectangle((int(sz / 2) + int(f / 2) + disparity * t, int(sz / 2) + y_pos,
                 int(sz / 2) - int(f / 2) + disparity * t, int(sz / 2) + y_pos - int(f)),
                fill=(0, 0, 0), outline=None)


# ls
def stereogramize(n, t=1, q=True): # 引数nはinner dotsの中央からの遷移量、引数tは視差の種類(交差or非交差)、引数qは遷移するドットを中央1点か3点か
    ...
    if q:
        ...
        n2 = 0
    # outer dots
    draw_dot(draw, int(ll*ln), t)
    draw_dot(draw, -int(ll*ln), t)
    # inner dots
    draw_dot(draw, int(ll*ln) - distance*15 + distance*n2, t)
    draw_dot(draw, int(ll*ln) - distance*10 + distance*n, t)
    draw_dot(draw, int(ll*ln) - distance*5 + distance*n2, t)

    fixation(draw)

    basename = os.path.basename(str(n) + 'ls' + str(t) + str(q) + '.png')
