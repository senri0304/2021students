...
def ls(t, li=[1], eye='b'):
    ...
    for i in li:
        draw.rectangle((int(sz / 2) - int(f / 2) + disparity*t, int(sz / 2) - (int(ll / 2) + 1 + 3*i)*t,
                        int(sz / 2) + int(f / 2) + disparity*t, int(sz / 2) - (int(ll / 2) + 1 + 3*i + int(f))*t),
                       fill=(0, 0, 0), outline=None)

    fixation(draw)

    basename = os.path.basename('ls' + str(li[len(li)-1]) + str(len(li)) + eye + '.png')
