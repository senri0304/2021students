...
files <- list.files('continuity_constraint/test_rds/data',full.names=T)
f <- length(files)
...
for (i in usi){
  ...
  g <- ggplot() +
       ...
       # エラーバーの追加
       stat_summary(aes(y=camp$cdt, x=camp$cnd), fun.data=mean_se, geom="errorbar", size=0.5, width=0.05) +
       # 試行ごとのデータを表示, -1が左眼、1が右眼にターゲットを提示
       geom_point(aes(y=camp$cdt, x=camp$cnd), position=position_jitter(width=0.02, height=0), alpha=0.4, shape=21) +
