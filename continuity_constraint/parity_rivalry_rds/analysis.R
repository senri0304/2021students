...
files <- list.files('continuity_constraint/parity_rivalry_rds/data',full.names=T)
f <- length(files)
...
for (i in usi){
  ...
  g <- ggplot() +
       ...
       # エラーバーの追加
       stat_summary(aes(y=camp$cdt, x=camp$cnd), fun.data=mean_se, geom="errorbar", size=0.5, width=max(camp$cnd)*0.05) +
       # 試行ごとのデータを表示, -1が左眼、1が右眼にターゲットを提示
       geom_point(aes(y=camp$cdt, x=camp$cnd), position=position_jitter(width=max(camp$cnd)*0.05, height=0), alpha=0.4, shape=21) +

       # ラベルの整形
       labs(subtitle=i, color='eye') + xlab('size') + theme(text = element_text(size = 20)) +
       ylim(0, 20) + ylab('cdt')
  print(g)
