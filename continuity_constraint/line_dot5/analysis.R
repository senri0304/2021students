...
files <- list.files('continuity_constraint/line_dot4/data',full.names=T)
f <- length(files)
...
discrimination <- function (x) {
  if (as.integer(substr(x, 2, 2) > 1)) {'array'}
  else {'alone'}
}

# split cnd
temp$cnd2 <- temp$cnd
temp$continuity <- lapply(temp$cnd, discrimination)
temp$cnd <- as.integer(substr(temp$cnd, 1, 1))


for (i in usi){
  camp <- subset(temp, sub==i & continuity=='alone')
  camp2 <- subset(temp, sub==i & continuity=='array')
  ...
  g <- ggplot() +
       ...
       # 試行ごとのデータを表示, -1が左眼、1が右眼にターゲットを提示
       geom_point(aes(y=camp2$cdt, x=camp2$cnd), position=position_jitter(width=0.3, height=0), alpha=0.4, shape=24, show.legend=FALSE) +

       # ラベルの整形
       labs(subtitle=i) + xlab('distance') + theme(text = element_text(size = 20)) +
       xlim(0, max(camp$cnd)+1) + ylim(0, 30)
  print(g)
