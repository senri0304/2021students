...
# scatter + number of bars + bar's orientation + numbering of images + .png
# 変数は、線分方位とナンバリングと検査眼
variation = [0, 45, 90]
numbering = list(range(0, 1))
test_eye = [1, -1]

# Replicate for repetition
variation2 = variation*len(numbering)*len(test_eye)
numbering2 = list(np.repeat(numbering, len(variation)))*len(test_eye)
...
random.seed(r)
sequence2 = random.sample(numbering2, len(variation2))
random.seed(r)
...
def set_polygon(seq, seq2, seq3):
    ...
    L = pyglet.resource.image('stereograms/scatter10' + str(seq) + str(seq2) + '.png')
