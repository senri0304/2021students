...
import os, pyglet, wave, struct, random
import numpy as np
...
sl = round(resolution * 0.2 / d_height)
...
# 線分が散布された図、2倍の大きさで作図する
def scatter(num, n, angle, seedx, seedy):
    random.seed(seedx)
    r1 = random.sample(range(int(-sz/4), int(sz/4), 4), k=num)
    random.seed(seedy)
    r2 = random.sample(range(int(-sz/4), int(sz/4), 4), k=num)
    r3 = np.radians(random.choices(range(-10, 10, 2), k=num))
    ...
    xs1 = r1 + int(sz)
    ys1 = r2 + int(sz)# + int(ll / 4)
    xs2 = r1 + int(sz)
    ys2 = r2 + int(sz)# - int(ll / 4)

    r3 = r3 + np.radians(angle)

    img = Image.new("RGB", (sz*2, sz*2), (lb, lb, lb))
    draw = ImageDraw.Draw(img)
    while len(xs1) > 0:
        draw.line((xs1[0] + np.cos(r3[0])*int(ll), ys1[0] + np.sin(r3[0])*int(ll),
                   xs2[0] - np.cos(r3[0])*int(ll), ys2[0] - np.sin(r3[0])*int(ll)), fill=(0, 0, 0), width=4)
    ...
    basename = os.path.basename('scat' + str(num) + str(angle) + str(n) + '.png')
    img.save(os.path.join(to_dir, basename), quality=100)

    # 保存した画像をリサイズしてアンチエイリアスをかける
    img_re = Image.open('stereograms/scat' + str(num) + str(angle) + str(n) + '.png')
    img_re.thumbnail((img.size[0]/2, img.size[1]/2), Image.ANTIALIAS)

    draw = ImageDraw.Draw(img_re)
    fixation(draw)

    img_re.save('stereograms/scatter' + str(num) + str(angle) + str(n) + '.png')
...
for i in range(0, 5):
    ...
    for j in [0, 45, 90]:
        scatter(5, i, j, rx, ry)
        scatter(10, i, j, rx, ry)
        scatter(20, i, j, rx, ry)
